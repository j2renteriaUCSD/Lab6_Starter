...
function getRecipesFromStorage() {
	...
	if (recipesArray == null) return new Array();
	return recipesArray;
}
...
function initFormHandler() {
	...
	formElement.addEventListener("submit", 
		function (event) {
			...
			const recipeDataObject = new Object();
			for (const data of formData.entries()) {
				recipeDataObject[data[0]] = data[1];	
			}
			// B6. TODO - Create a new <recipe-card> element
			const recipeCardElement = document.createElement("recipe-card");
			// B7. TODO - Add the recipeObject data to <recipe-card> using element.data
			recipeCardElement.data = recipeDataObject;
			// B8. TODO - Append this new <recipe-card> to <main>
			document.querySelector("main").append(recipeCardElement);
			// B9. TODO - Get the recipes array from localStorage, add this new recipe to it, and
			//            then save the recipes array back to localStorage
			const recipesArray = getRecipesFromStorage();
			recipesArray.push(recipeDataObject);
